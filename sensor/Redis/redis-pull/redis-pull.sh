...
function write_data(){
        #POP 10 events from Redis
        echo "$(/source/Redis/redis-cli -r 100 LPOP "$1" | iconv -c | tr -d '\0')" | sed '/^$/d' | sed "/^$ERROR_1$/d; /^$ERROR_2$/d" >> "${DIRECTORY[$i]}$CURRENT_TIME.log"
}
